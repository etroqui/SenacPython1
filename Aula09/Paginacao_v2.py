...
v_arquivo_saida = 'ListaSenacComPaginacao15.xlsx'
...
def getDadosSoup(p_url):
    ...
    if d_resposta.status_code == 200:
        ...
        #se eu usar -1 em uma lista, ele retorna o ultimo item da lista, caso eu nao saiba o tamanho da lista
        #d_pagina_total      = d_pagina[-1].text.strip()[12:13]
        d_pagina_total      = d_pagina[-1].text.strip().split(' ')[-1]
...
def ConsultaSitePagueMenos(p_url):
    ...
    if resposta.status_code == 200:
        ...
        for produto in v_produtos:
            v_fabricante = produto.find('span',class_='font-size-11 text-primary font-weight-bold').text
            
            if produto.find('span',class_='discount'):
                v_desconto = produto.find('span',class_='discount').text
            else:
                v_desconto = ""
...
v_url_base      = "https://www.superpaguemenos.com.br/hortifruti/"
